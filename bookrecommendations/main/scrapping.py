        ...
        for book in books:
            ...
            if (Book.objects.filter(title=title).first() is None and synopsis != ""):
                Book.objects.create(title=title, bookURL=bookURL, author=author, coverURL=imageURL, npages=npages,
                                    editorial=editorial, language=lengua, category="Contemporánea", binding=binding,
                                    synopsis=synopsis)


def scrapBlack():
    total = 0
    f = urllib.request.urlopen('https://www.casadellibro.com/libros/novela-negra/126000000')
    html = f.read()
    f.close()
    soup = BeautifulSoup(html, 'lxml')
    pages = soup.find('span', class_='fnt09im').text.split(" ")[3]
    for i in range(1):
        f = urllib.request.urlopen('https://www.casadellibro.com/libros/novela-negra/126000000/p' + str(i + 1))
        html = f.read()
        f.close()
        soup = BeautifulSoup(html, 'lxml')
        books = soup.find_all('div', class_='mod-list-item')
